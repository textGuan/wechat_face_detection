...
settingsBtn.addEventListener('click', (e) => {
    ...
    if (settingsPanel.classList.contains('visible')) {
        const currentShortcut = ipcRenderer.sendSync('get-current-shortcut')
...
function buildShortcutString(event) {
    ...
    else if (event.ctrlKey) keys.push('Control')
...
document.addEventListener('keydown', (e) => {
    if (isRecordingShortcut) {
        ...
        if (shortcut && shortcut.includes('+')) {
            ...
            const success = ipcRenderer.sendSync('set-shortcut', shortcut)
            if (success) {
                stopRecording()
            } else {
                shortcutInput.value = '无效快捷键'
                setTimeout(() => {
                    const currentShortcut = ipcRenderer.sendSync('get-current-shortcut')
                    shortcutInput.value = currentShortcut
                    stopRecording()
...
clearShortcut.addEventListener('click', () => {
    const defaultShortcut = process.platform === 'darwin' ? 'Command+Space' : 'Control+Space'
    const success = ipcRenderer.sendSync('set-shortcut', defaultShortcut)
    if (success) {
